...
import { peekAudioEngine, isReducedAudio } from './audio-engine'
...
export function playImpact(
  ...
  const workletTypes: ImpactCategory[] = ['bumper', 'peg', 'flipper', 'drain']
  let workletPlayed = false
  if (workletTypes.includes(category)) {
    const engine = peekAudioEngine()
    if (engine?.isWorkletReady()) {
      const n = normalizeImpactVelocity(velocity)
      workletPlayed = engine.playVoice({
        type: category as 'bumper' | 'peg' | 'flipper' | 'drain',
        velocity: n,
  ...
  if (sampleHelpers?.usesSampleBank()) {
    ...
    if (key) {
      ...
      if (sampleHelpers.playLocalSampleKey(key, vol, options.position, rate)) {
        if (workletPlayed) return
        return
      }
    }
  }
  ...
  const reducedAudio = isReducedAudio()
