...
import type { ExtendedPachinballEventMap } from '../game/event-bus-extensions'

export type ExtendedEventName = keyof ExtendedPachinballEventMap
export type ExtendedEventPayload<K extends ExtendedEventName> = ExtendedPachinballEventMap[K]
...
export class ObstacleEventBusIntegration {
  ...
  emitSpinnerHit(spinnerId: string, position: { x: number; y: number; z: number }, force: number): void {
    (this.eventBus as any).emit?.('bumper:spinner:hit', {
      spinnerId,
      position,
      force
    })
  ...
  emitSpinnerRotation(spinnerId: string, rotationSpeed: number, progress: number): void {
    (this.eventBus as any).emit?.('bumper:spinner:rotation', {
      spinnerId,
      rotationSpeed,
      progress
    })
  ...
  emitSpinnerFullRotation(spinnerId: string, completions: number): void {
    (this.eventBus as any).emit?.('bumper:spinner:full-rotation', {
      spinnerId,
      completions
    })
  }
  ...
  emitTrapBallCaptured(trapId: string, ballId: string, position: { x: number; y: number; z: number }): void {
    (this.eventBus as any).emit?.('trap:ball:captured', {
      trapId,
      ballId,
      position
    })
  ...
  emitTrapBallReleased(trapId: string, ballId: string, exitVelocity: { x: number; y: number; z: number }): void {
    (this.eventBus as any).emit?.('trap:ball:released', {
      trapId,
      ballId,
      exitVelocity
    })
  ...
  emitLauncherCharged(launcherId: string, chargeLevel: number): void {
    (this.eventBus as any).emit?.('launcher:charged', {
      launcherId,
      chargeLevel
    })
  }
  ...
  emitLauncherFired(launcherId: string, ballId: string, force: { x: number; y: number; z: number }, chargeRatio: number): void {
    (this.eventBus as any).emit?.('launcher:fired', {
      launcherId,
      ballId,
      force,
      chargeRatio
  ...
  emitLauncherTriggered(launcherId: string, ballId: string): void {
    (this.eventBus as any).emit?.('launcher:triggered', {
      launcherId,
      ballId
    })
  }
  ...
  emitGateTriggered(gateId: string, position: { x: number; y: number; z: number }): void {
    (this.eventBus as any).emit?.('gate:triggered', {
      gateId,
      position
    })
  }
  ...
  emitGateOpened(gateId: string, duration: number): void {
    (this.eventBus as any).emit?.('gate:opened', {
      gateId,
      duration
    })
  }
  ...
  emitGateClosed(gateId: string): void {
    (this.eventBus as any).emit?.('gate:closed', {
      gateId
    })
  }

  emitGateStateChanged(gateId: string, isOpen: boolean): void {
    (this.eventBus as any).emit?.('gate:state-changed', {
      gateId,
      isOpen
    })
  }
  ...
  emitDropTargetHit(targetId: string, bankId: string, position: { x: number; y: number; z: number }): void {
    (this.eventBus as any).emit?.('drop-target:hit', {
      targetId,
      bankId,
      position
    })
  ...
  emitDropTargetBankComplete(bankId: string, targetCount: number): void {
    (this.eventBus as any).emit?.('drop-target:bank-complete', {
      bankId,
      targetCount
    })
  }
  ...
  emitDropTargetBankReset(bankId: string): void {
    (this.eventBus as any).emit?.('drop-target:bank-reset', {
      bankId
    })
  }
  ...
  emitComboStarted(comboCount: number): void {
    (this.eventBus as any).emit?.('combo:started', {
      comboCount
    })
  }

  emitComboExtended(comboCount: number): void {
    (this.eventBus as any).emit?.('combo:extended', {
      comboCount
    })
  }

  emitComboBroken(finalComboCount: number): void {
    (this.eventBus as any).emit?.('combo:broken', {
      finalComboCount
    })
  }
