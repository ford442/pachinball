...
export class NanoLoomFeeder {
  ...
  /**
   * Optional glTF insert overlay — visual-only; Rapier frame colliders stay code-authored.
   */
  async loadInsertGltf(tier: QualityTier): Promise<void> {
    try {
      const result = await loadInsertGltfForPreset(this.scene, NANO_LOOM_INSERT_GLTF, tier)
      this.insertContainer = result.container
      attachInsertMeshes(result.meshes, this.position)
      this.setProceduralMeshesVisible(false)
      console.info(`[NanoLoom] Insert glTF loaded (${result.lod}): ${result.url}`)
    } catch (err) {
      console.warn('[NanoLoom] Insert glTF unavailable, using procedural mesh:', err)
    }
  }

  private setProceduralMeshesVisible(visible: boolean): void {
    this.proceduralMeshesVisible = visible
    this._frameMesh?.setEnabled(visible)
    this._intakeMesh?.setEnabled(visible)
    for (const pin of this.pins) pin.setEnabled(visible)
  }
