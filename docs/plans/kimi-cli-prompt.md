...
**Goal**: Fully decouple sound from direct calls in `src/game.ts`. Make SoundSystem reactive via EventBus subscriptions (matching the new architecture used by DisplaySystem, EffectsSystem, GameStateManager).
...
1. Locate the sound implementation:
   ...
   - Check how it's currently initialized in `src/game.ts`.
...
4. Verify:
   ...
   - No new direct calls to sound from `src/game.ts`.
...
- Update `src/game.ts` to read from `config` instead of hardcoding.
